...
export default function MedicalDashboard() {
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white font-sans selection:bg-[#4bc7c5] selection:text-black relative pb-20 md:pb-12">
      ...
      {/* Header */}
      <header className="sticky top-0 z-40 flex flex-wrap items-center justify-between px-4 py-4 md:px-6 md:py-5 border-b border-[#333] bg-[#0a0a0a]/90 backdrop-blur-md gap-4">
        ...
        {/* Header Icons - Hidden on very small screens or adjusted */}
        <div className="flex items-center gap-4 md:gap-6 text-[10px] md:text-xs font-mono text-gray-400">
      ...
      {/* Main Content */}
      <main className="relative z-10 max-w-6xl mx-auto p-4 md:p-6 grid grid-cols-1 lg:grid-cols-12 gap-4 md:gap-6 mt-2 md:mt-4">
        
        {/* Left Column: Device Visualization */}
        <div className="lg:col-span-8 flex flex-col gap-4 md:gap-6">
          
          {/* Glasses Visualizer */}
          <div className="bg-[#111] rounded-3xl border border-[#333] relative overflow-hidden flex flex-col items-center justify-center min-h-[300px] md:min-h-[450px] p-4 md:p-8 group shadow-[0_0_50px_rgba(75,199,197,0.05)]">
            <div className="absolute top-4 left-4 md:top-6 md:left-6 flex flex-col gap-1 z-20">
              <span className="text-[10px] text-gray-500 uppercase">Status</span>
              <div className="flex items-center gap-2">
                 <span className="w-2 h-2 rounded-full bg-[#4bc7c5] animate-pulse shadow-[0_0_8px_#4bc7c5]"></span>
                 <span className="text-[#4bc7c5] font-mono text-xs md:text-sm glow-text">ACTIVE THERAPY</span>
              </div>
            </div>

            {/* SVG Glasses */}
            <div className="relative w-full max-w-2xl aspect-[16/9] flex items-center justify-center scale-90 md:scale-100">
               ...
               <div className="absolute right-[15%] top-[68%] w-[90px] h-[50px] md:w-[110px] md:h-[70px] z-20">
                  ...
                  <div className="w-full h-full bg-black/90 border border-[#4bc7c5]/40 rounded-lg overflow-hidden relative p-2 shadow-[0_0_20px_rgba(75,199,197,0.15)]">
                      <PulseWave />
                      <div className="absolute bottom-1 right-2 text-[8px] md:text-[9px] text-[#4bc7c5] font-mono flex items-center gap-1">
            ...
            <div className="absolute bottom-4 w-full px-4 md:bottom-6 md:px-8 flex justify-between text-[10px] md:text-xs font-mono text-gray-500">
          ...
          {/* Graph Card */}
          <div className="h-auto min-h-[160px] md:h-48 bg-[#111] rounded-3xl border border-[#333] p-4 md:p-6 flex flex-col sm:flex-row items-center justify-between shadow-[0_5px_15px_rgba(0,0,0,0.3)] gap-4">
             <div className="flex flex-col gap-2 md:gap-3 w-full sm:w-auto">
                <span className="text-xs md:text-sm text-gray-400 font-medium uppercase tracking-wider">Current Intensity</span>
                <div className="flex items-baseline gap-2">
                   <span className="text-3xl md:text-4xl font-bold text-white font-mono">1.2</span>
                   <span className="text-base md:text-lg text-[#4bc7c5] font-mono">mA</span>
                </div>
             ...
             {/* Graph Bars - Responsive Width */}
             <div className="flex items-end gap-1 h-20 md:h-28 w-full sm:w-56 p-2 bg-[#0a0a0a] rounded-xl border border-[#222] inner-shadow">
        ...
        <div className="lg:col-span-4 flex flex-col gap-4">
          ...
          {/* Goal Card */}
          <div className="bg-[#151515] rounded-3xl border border-[#333] p-4 md:p-6 flex flex-col justify-between gap-6 shadow-[0_5px_15px_rgba(0,0,0,0.3)]">
             <div>
                ...
                <div className="relative w-40 h-40 md:w-48 md:h-48 mx-auto">
                   <svg className="w-full h-full transform -rotate-90 drop-shadow-[0_0_10px_rgba(75,199,197,0.2)]">
                      <circle cx="50%" cy="50%" r="45%" stroke="#222" strokeWidth="14" fill="none" />
                      <motion.circle cx="50%" cy="50%" r="45%" stroke="url(#progressGradient)" strokeWidth="14" fill="none" strokeDasharray={2 * Math.PI * 88} strokeDashoffset={2 * Math.PI * 88} animate={{ strokeDashoffset: 2 * Math.PI * 88 * (1 - 0.65) }} transition={{ duration: 2.5, ease: "easeOut", delay: 0.5 }} strokeLinecap="round" />
                      <defs>
                   ...
                   <div className="absolute inset-0 flex flex-col items-center justify-center">
                      <span className="text-3xl md:text-4xl font-bold text-white">65<span className="text-xl md:text-2xl text-[#4bc7c5]">%</span></span>
                      <span className="text-[10px] md:text-xs text-gray-500 uppercase tracking-widest mt-1">COMPLETED</span>
                   </div>
                </div>
             </div>
             <button className="w-full py-4 rounded-2xl bg-gradient-to-r from-[#4bc7c5] to-[#3baea2] hover:from-[#42b5b3] hover:to-[#359e93] text-black font-bold transition-all transform hover:scale-[1.02] flex items-center justify-center gap-2 shadow-[0_0_20px_rgba(75,199,197,0.4)] text-sm md:text-base">
                PAUSE THERAPY
             </button>
          </div>
        </div>
  ...
  return (
    <>
      <button 
        ...
        className="fixed bottom-4 right-4 md:bottom-6 md:right-6 z-50 p-3 md:p-4 rounded-full bg-[#4bc7c5] hover:bg-[#3baea2] text-black shadow-[0_0_20px_rgba(75,199,197,0.4)] transition-all transform hover:scale-110 flex items-center justify-center"
      >
        {isOpen ? <X size={24} className="md:w-7 md:h-7" strokeWidth={2.5} /> : <Bot size={24} className="md:w-7 md:h-7" strokeWidth={2.5} />}
      </button>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div 
            ...
            className="fixed bottom-20 right-4 md:bottom-24 md:right-6 z-50 w-[calc(100vw-32px)] md:w-[350px] h-[60vh] md:h-[500px] bg-[#0a0a0a]/95 backdrop-blur-xl border border-[#333] rounded-3xl shadow-2xl flex flex-col overflow-hidden"
          >
            <div className="p-4 border-b border-[#222] bg-[#111] flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-[#4bc7c5]/20 flex items-center justify-center border border-[#4bc7c5]/30 shrink-0">
            ...
            <div className="p-4 border-t border-[#222] bg-[#111]">
              <div className="flex items-center gap-2 bg-[#050505] border border-[#333] rounded-full px-4 py-2 focus-within:border-[#4bc7c5] transition-colors">
                <input 
                  ...
                  className="flex-1 bg-transparent text-sm text-white focus:outline-none placeholder:text-gray-600 min-w-0"
                />
                <button 
                  onClick={handleSend}
                  className={`p-2 rounded-full transition-all shrink-0 ${input.trim() ? 'text-[#4bc7c5] hover:bg-[#4bc7c5]/10' : 'text-gray-600 cursor-not-allowed'}`}
                >
                  <Send size={18} />
                </button>
              </div>
...
function KPICard({ icon, label, value, unit, active = false, footer }: any) {
   return (
      <div className={`p-4 md:p-6 rounded-3xl border transition-all duration-300 ${active ? 'bg-gradient-to-br from-[#4bc7c5]/20 to-[#151515] border-[#4bc7c5]/40 shadow-[0_0_30px_rgba(75,199,197,0.15)]' : 'bg-[#151515] border-[#333] hover:border-[#444] shadow-[0_5px_15px_rgba(0,0,0,0.3)]'}`}>
         ...
         <div className="mt-2 flex items-baseline gap-2">
            ...
            <span className="text-xs md:text-sm text-gray-400 font-mono font-medium">{unit}</span>
         </div>
   ...
   return (
      <div className="flex items-center justify-between h-full w-full overflow-hidden px-1 gap-[1px]">
         {[...Array(24)].map((_, i) => (
            <motion.div
               key={i}
               className="w-[2px] md:w-[3px] bg-[#4bc7c5] rounded-full"
